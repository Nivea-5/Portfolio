                ...
                section.elements.map((element) => {
                    switch (element.type) {
                        case PossibleElemType.image:
                            return <img key={element.id} src={element.content} alt={"image"} className={"md:w-1/4 w-full"}/>
                        case PossibleElemType.titre:
                            return <h1 key={element.id} className={"md:w-2/3 text-center"}>{element.content}</h1>
